...
var ann_canvas = document.createElement('canvas'); //Create a ann_canvas element
//Set ann_canvas width/height
ann_canvas.style.width='100%';
ann_canvas.style.height='100%';
//Set ann_canvas drawing area width/height
ann_canvas.width = window.innerWidth;
ann_canvas.height = window.innerHeight;
//Position ann_canvas
ann_canvas.style.position='absolute';
ann_canvas.style.left=0;
ann_canvas.style.top=0;
ann_canvas.style.zIndex=100000;

ann_canvas.style.pointerEvents='none'; //Make sure you can click 'through' the ann_canvas
var parentDiv = document.getElementById("EEGplot");
parentDiv.appendChild(ann_canvas);
var ann_ctx = ann_canvas.getContext('2d');
...
t_button.onclick = function(){
  myHH = $('input[id = "myHH"]').val();
  myMM = $('input[id = "myMM"]').val();
  mySS = $('input[id = "mySS"]').val();

  datetime_temp = edf.date_time.clone();
  var t_beg_temp = edf.t_beg;
  var st_date_time_temp = datetime_temp.add(t_beg_temp, 'seconds');

  stHH = parseInt(st_date_time_temp.format("HH"));
  stMM = parseInt(st_date_time_temp.format("mm"));
  stSS = parseInt(st_date_time_temp.format("ss"));
...
function t_secs(time){
  return (parseInt(time.substring(0,2))*3600 +parseInt(time.substring(3,5))*60+parseInt(time.substring(6,8)));
}
...
function readEEG() {
    first_draw = true;
    ann_ctx.clearRect(0,0,canvas.width,canvas.height); //clear canvas
  ...
  //  } else {
    ...
    if(parsedata.length){
      ...
      var ann_diff;
      if (diff == [0,0,0]){
        ann_diff = [parseInt(stHH),parseInt(stMM),parseInt(stSS)];
      }
      else{
        ann_diff = [parseInt(myHH),parseInt(myMM),parseInt(mySS)];
      }
      parsedata.forEach(function draw_ann(row){
        if (row!=""){

        row_st = addTime(row[0].substring(0,8),ann_diff);
        row_ed = addTime(row[1].substring(0,8),ann_diff);

        var le_st = t_secs(document.getElementById('startWindowtime').innerHTML);
        var le_ed = t_secs(document.getElementById('endWindowtime').innerHTML);
        if ((t_secs(row_st)>= le_st) && (t_secs(row_st)<= le_ed)){
          ann_ctx.beginPath();
          var dur = parseInt(document.getElementById('wd').innerHTML);
          ann_width = ann_canvas.width * ((t_secs(row_ed)+parseInt(row[1].substring(9))/1000) - (t_secs(row_st)+parseInt(row[0].substring(9))/1000))/ (dur);
          var an_rec_st = ann_canvas.width * ((t_secs(row_st)+parseInt(row[0].substring(9))/1000)-(le_st))/dur;
          ann_ctx.rect(an_rec_st,0,ann_width,ann_canvas.height);
          ann_ctx.strokeStyle = 'black';
          ann_ctx.lineWidth = 0.5;
          ann_ctx.fillStyle = "rgba(150, 50, 50, 0.4)";
          ann_ctx.fill()
          ann_ctx.stroke();
    ...
    if(trudata.length){
      ...
      if (tru_fs){
        ...
        sub_ar = [parseInt(tru_fs.substring(0,2)),parseInt(tru_fs.substring(3,5)),parseInt(tru_fs.substring(6))]
      }
      else{
        sub_ar = [parseInt(stHH),parseInt(stMM),parseInt(stSS)]
      }

      var temp_diff = ann_diff.map(function(item, index) {
        // In this case item correspond to currentValue of array a,
        // using index to get value from array b
        return item - sub_ar[index];
        })
        ann_diff = temp_diff;

      trudata.forEach(function draw_ann(row){
        if (row!=""){
        ...
        if ((t_secs(row_st)>= le_st) && (t_secs(row_st)<= le_ed)){
          ...
          ann_ctx.strokeStyle = 'black';
          ann_ctx.lineWidth = 0.5;
          ann_ctx.fillStyle = "rgba(100, 100, 250, 0.4)";
          ann_ctx.fill()
          ann_ctx.stroke();
